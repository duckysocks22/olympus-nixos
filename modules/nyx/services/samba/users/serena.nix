{ config, ... }:
{
  ...
  system.activationScripts = {
    ...
    init_smbpasswd.text = ''
      /run/current-system/sw/bin/printf "$(/run/current-system/sw/bin/cat ${config.sops.secrets."samba/serena".path})\n$(/run/current-system/sw/bin/cat ${config.sops.secrets."samba/serena".path})\n" | /run/current-system/sw/bin/smbpasswd -sa serena
    '';
  };
}
