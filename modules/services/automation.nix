{ inputs, self, ... }: {
  ...
  flake.nixosModules.podman = { inputs, ... }: {
    ...
    users.users.server.extraGroups = [ "podman" ];
  };
  ...
  flake.nixosModules.govee2mqtt = { pkgs, lib, ... }: {
    ...
    systemd.services."podman-govee2mqtt" = {
      ...
      partOf = [
        "podman-compose-govee2mqtt-root.target"
      ];
      wantedBy = [
        "podman-compose-govee2mqtt-root.target"
      ];
    ...
    systemd.targets."podman-compose-govee2mqtt-root" = {
      unitConfig = {
