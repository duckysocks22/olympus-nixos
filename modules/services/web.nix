{ inputs, self, ... }: {
  ...
  flake.nixosModules.forgejo-runner = { pkgs, lib, config, ... }: {
  ...
  flake.nixosModules.atticd = { inputs, pkgs, pkgs-unstable, config, lib, ... }: {
    imports = [
  ...
  flake.nixosModules.caddy = { config, pkgs, ... }: let
    puppygirls = inputs.self.packages.${pkgs.system}.puppygirls;
  in {
    services.caddy = {
      enable = true;
  ...
  flake.nixosModules.avahi = { util, pkgs, lib, ... }: {
    services.avahi = {
      enable = true;
      openFirewall = true;
      allowInterfaces = [
