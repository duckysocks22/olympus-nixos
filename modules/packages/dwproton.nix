{ inputs, ...}: {
  ...
  perSystem = { pkgs, lib, ... }: {
    packages.dwproton = pkgs.stdenvNoCC.mkDerivation (finalAttrs: {
      pname = "dwproton-bin";
      ...
      preFixup = ''
        substituteInPlace "$steamcompattool/compatibilitytool.vdf" \
          --replace-fail "${finalAttrs.version}" "dwproton"
      '';
      ...
      */
      passthru.updateScript = pkgs.writeScript "update-dwproton" ''
        #!/usr/bin/env nix-shell
        #!nix-shell -i bash -p curl jq common-updater-scripts
        repo="https://dawn.wine/dawn-winery/dwproton/releases"
